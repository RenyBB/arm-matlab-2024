...
const options = {
  sheetId: "1kf7MJK7E15YG0tq4ICGxgbQ5hYtYWNs8Ox2_XBfNO1U",
  sheetNumber: 1,
...
GSheetReader(
  ...
  (results) => {
    ...
    const prog_lang = keys[1];
    const methods_interest = keys[2];
    const os = keys[3];
    // process languages
    var langs = [];
    results.forEach((result) => {
      langs += result[prog_lang] + ", ";
    });

    langs = langs
      .split(", ")
      .filter((x) => (x == "" ? NaN : x))
      .filter((x) => (x == "undefined" ? NaN : x));
    ...
    var methods = [];
    results.forEach((result) => {
      methods += result[methods_interest] + ", ";
    });
    ...
    function processData(input) {
      var input_count = _.countBy(input);
      x_ = Object.entries(input_count).map((x) => x[0]);
      values_ = Object.entries(input_count).map((x) => x[1]);
      var data = [];
      for (let i = 0; i < x_.length; i++) {
        data[i] = { x: x_[i], value: values_[i] };
      }
      return data;
    }

    function responseResponses(x) {
      return x == 1 ? `${x} response` : `${x} responses`;
    }

    langs = processData(langs);
    let el = document.getElementById("langs");
    el.innerHTML = langs
      .map(
        (x) => `<strong>${x.x}:</strong> ${responseResponses(x.value)}<br />`
      )
      .join("");

    methods = processData(methods);
    console.log(methods);
    el = document.getElementById("methods");
    el.innerHTML = methods
      .map(
        (x) =>
          `<strong>${x.x}:</strong> ${responseResponses(
            x.value
          )}</strong><br />`
      )
      .join("");

    oses = processData(oses);
    console.log(oses);
...
},{"g-sheets-api":4,"lodash":5}],2:[function(require,module,exports){
"use strict";
...
},{"node-fetch":6}],3:[function(require,module,exports){
"use strict";
